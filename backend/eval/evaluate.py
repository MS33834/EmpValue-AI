...
用法：
    ...
    python backend/eval/evaluate.py --mock

    # 指定档位
    python backend/eval/evaluate.py --tier L0
"""
...
class MockProvider(BaseProvider):
    ...
    def _classify(content: str) -> tuple[str, int]:
        """根据原始内容判断 tone 与分数"""
        negative_signals = ["延期", "崩溃", "迟到", "质量不高", "未自测", "差距较大", "阻塞"]
...
def build_mock_evaluation(
    ...
    if tone == "positive":
        ...
        strengths = ["主导完成核心模块重构，性能提升40%", "主动辅导新人，提升团队整体代码质量"]
        growth = [
            {
                "dimension": "技术交付",
                "score": 90,
    ...
    return {
        ...
        "employee_view": {
            ...
            "strengths": strengths,
            "growth_areas": growth,
            "next_week_focus": ["继续保持优势项", "重点改进已识别短板", "主动同步关键进展"],
        },
        "manager_view": {
...
def evaluate_case(case: dict, eval_result: dict, skip_contains: bool = False, skip_score: bool = False) -> dict:
    contains_result = (True, "skipped") if skip_contains else check_contains(eval_result, case["expected_contains"])
    score_result = (True, "skipped") if skip_score else check_overall_score_range(eval_result, case["expected_overall_score_range"])
    results = {
...
async def run_dataset(
    ...
    for case in dataset:
        ...
        print(f"[{status}] {case['employee_id']} {case['period']} ({case.get('archetype', '')})")
    return results
...
def compare_versions(
    ...
    """
    current_by_key = {
        (r["employee_id"], r["period"]): r for r in current_results
    }
    deltas = []
    regressions = []
...
async def main():
    parser = argparse.ArgumentParser(description="EmpValue-AI LLM 回归评估")
    parser.add_argument("--mock", action="store_true", help="使用 Mock Provider 跑通流程")
    ...
    if args.compare:
        ...
        print(f"目标版本 {version} 通过: {report['version_pass']}/{report['version_total']}")
        print(f"pass 增量: {report['pass_delta']:+d}")
        if report["has_regression"]:
            print(f"⚠️  检测到 {len(report['regressions'])} 处回归，不建议发布该 Prompt 变更：")
