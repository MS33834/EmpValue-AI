...
# 负面词列表（与 evaluate.py 语义一致）：员工视图不应出现打击性词汇
NEGATIVE_WORDS = [
...
class LLMJudge:
    ...
    async def _score_with_llm(self, prompt: str, dimension: str) -> dict:
        ...
        completion = await self.provider.chat_completion(
...
class MockJudge(LLMJudge):
    ...
    async def judge_evidence(self, eval_result: dict, raw_inputs: list) -> dict:
        ...
        if ratio == 1.0:
            ...
            reason = f"仅 {matched}/{len(evidence_items)} 条证据可溯源，存在编造证据风险"
        return {"score": score, "reason": reason}
    ...
    async def judge_tone_separation(self, eval_result: dict) -> dict:
        ...
        reason = "；".join(reasons) if reasons else "员工视图建设性良好，管理视图风险标注完整"
        return {"score": score, "reason": reason}

    async def judge_hallucination(self, eval_result: dict, raw_inputs: list) -> dict:
        growth_areas = eval_result.get("employee_view", {}).get("growth_areas", []) or []
        if not growth_areas:
...
async def run_dataset_with_judge(
    ...
    for case in dataset:
        try:
            ...
            judge_result = await judge.judge_all(eval_result, case.get("raw_inputs", []))
            entry = {
...
async def main():
    parser = argparse.ArgumentParser(description="EmpValue-AI LLM-as-Judge 评估")
    parser.add_argument("--dataset", default=None, help="数据集路径，默认 eval/dataset.json")
    parser.add_argument("--mock", action="store_true", help="使用 MockJudge（规则启发式，不调真实 LLM）")
