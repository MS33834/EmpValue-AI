...
def test_detect_hardware_psutil_failure(monkeypatch, caplog):
    ...
    monkeypatch.setattr(mr.subprocess, "check_output", MagicMock(side_effect=FileNotFoundError("no nvidia-smi")))
...
def test_detect_hardware_no_gpu_at_all(monkeypatch):
    ...
    monkeypatch.setattr(mr.subprocess, "check_output", MagicMock(side_effect=FileNotFoundError("none")))
    hw = ModelRouter._detect_hardware()
...
def test_hardware_report_includes_recommended_tier():
    ...
    router = ModelRouter(settings)
    router._hardware = {"vram_gb": 8, "ram_gb": 16, "gpu_count": 1, "gpu_names": ["RTX"]}
