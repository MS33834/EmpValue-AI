...
def _eval_data(evaluation_id="EVAL-1", employee_id="E1001", period="2026-W01", score=80.0):
    """构造一份合法的评估数据字典"""
    return {
...
async def _make_user(service, user_id="E1001", role="employee"):
    return await service.create_user({"user_id": user_id, "name": user_id, "role": role})
...
async def test_update_status_non_approved_with_approver_sets_approver(service, db_session):
...
async def test_get_employee_history_returns_payloads(service, db_session):
    await _make_user(service)
    await service.add_memory("E1001", {"period": "2026-W01", "summary": "第一周"})
    await service.add_memory("E1001", {"period": "2026-W02", "summary": "第二周"})
    await db_session.flush()

    history = await service.get_employee_history("E1001")
    assert len(history) == 2
    periods = {h["period"] for h in history}
    assert periods == {"2026-W01", "2026-W02"}


async def test_get_employee_history_excludes_given_period(service, db_session):
    await _make_user(service)
    await service.add_memory("E1001", {"period": "2026-W01", "summary": "第一周"})
    await service.add_memory("E1001", {"period": "2026-W02", "summary": "第二周"})
    await db_session.flush()

    history = await service.get_employee_history("E1001", period="2026-W02")
    assert len(history) == 1
    assert history[0]["period"] == "2026-W01"


# ---------------- query_company_kb ----------------


async def test_query_company_kb_scores_and_ranks(service, db_session):
    await service.create_kb_doc(
        {"kb_id": "KB-1", "title": "执行力 标准", "content": "执行力 协作", "metadata": {"tag": "a"}}
    )
    await service.create_kb_doc(
        {"kb_id": "KB-2", "title": "创新", "content": "创新 能力", "metadata": {}}
    )
    await db_session.flush()

    # 查询词命中 KB-1（执行力），不命中 KB-2
    results = await service.query_company_kb("执行力", top_k=5)
    assert len(results) == 1
    assert results[0]["kb_id"] == "KB-1"
    assert results[0]["metadata"] == {"tag": "a"}


async def test_query_company_kb_top_k_limit(service, db_session):
    for i in range(3):
        await service.create_kb_doc(
            {"kb_id": f"KB-{i}", "title": "执行力", "content": "执行力", "metadata": {}}
        )
    await db_session.flush()

    results = await service.query_company_kb("执行力", top_k=2)
    assert len(results) == 2


async def test_query_company_kb_no_match_returns_empty(service, db_session):
    await service.create_kb_doc(
        {"kb_id": "KB-1", "title": "执行力", "content": "执行力", "metadata": {}}
    )
    await db_session.flush()

    assert await service.query_company_kb("不存在的词") == []
...
async def test_create_kb_doc_persists_fields(service, db_session):
    doc = await service.create_kb_doc(
        {"kb_id": "KB-X", "title": "价值观", "content": "客户第一", "metadata": {"v": 1}}
    )
    await db_session.flush()
