...
def test_init_embedding_falls_back_to_dummy_on_failure(caplog):
    ...
    with patch("memory.vector_store.EmbeddingClient", side_effect=RuntimeError("init boom")):
        emb = _init_embedding(settings)
    assert isinstance(emb, DummyEmbeddingFunction)
...
async def test_memory_add_and_get_history_round_trip(memory_store):
    """写入记忆后应能检索到"""
    await memory_store.add_memory("E1001", {"period": "2026-W01", "summary": "完成重构"})
    history = await memory_store.get_employee_history("E1001", period="2026-W99", limit=5)
...
async def test_memory_history_excludes_current_period(memory_store):
    ...
    history = await memory_store.get_employee_history("E2001", period="2026-W02", limit=10)
...
async def test_memory_add_memory_without_period_defaults_unknown(memory_store):
    ...
    history = await memory_store.get_employee_history("E3001", period="2026-W10", limit=5)
    assert len(history) == 1
