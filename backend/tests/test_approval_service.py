...
async def _seed_eval(eval_service, db_session, evaluation_id="EVAL-1", status=EvaluationStatus.AI_DRAFTED):
    """创建用户并写入一份评估，返回 evaluation_id"""
    await eval_service.create_user({"user_id": "E1001", "name": "E1001", "role": "employee"})
...
def test_can_transition_returns_true_for_valid_action():
    ...
    assert ApprovalService.can_transition(EvaluationStatus.AI_DRAFTED, "approve") is True


def test_can_transition_returns_false_for_invalid_action():
    """非法 action 应返回 False"""
    assert ApprovalService.can_transition(EvaluationStatus.AI_DRAFTED, "foobar") is False
...
async def test_transition_approve_updates_status_and_approval_info(approval_service, eval_service, db_session):
    """approve 成功后 status 更新为 approved，并设置 approved_at 与 approver_id"""
    evaluation_id = await _seed_eval(eval_service, db_session, status=EvaluationStatus.AI_DRAFTED)
...
async def test_transition_reject_updates_status(approval_service, eval_service, db_session):
...
async def test_transition_request_hr_review_with_approver_sets_approver(approval_service, eval_service, db_session):
...
async def test_appeal_from_approved_resets_approval_info(approval_service, eval_service, db_session):
...
async def test_transition_illegal_action_raises(approval_service, eval_service, db_session):
...
async def test_transition_legal_action_unsupported_by_status_raises(approval_service, eval_service, db_session):
...
async def test_duplicate_approve_on_approved_raises(approval_service, eval_service, db_session):
...
async def test_get_history_returns_ordered_actions(approval_service, eval_service, db_session):
...
async def test_get_history_empty_when_no_actions(approval_service, eval_service, db_session):
