...
def test_init_falls_back_to_cloud_then_openai_keys():
    ...
    s1 = Settings(cloud_api_key="cloud-key", cloud_base_url="https://cloud.example.com/v1")
    c1 = EmbeddingClient(s1)
    assert c1._has_real_key is True

    s2 = Settings(openai_api_key="openai-key", openai_base_url="https://api.openai.com/v1")
...
async def test_embed_empty_input_returns_empty_list():
    ...
    settings = Settings(embedding_api_key="k", embedding_base_url="https://x.example.com/v1")
    client = EmbeddingClient(settings)
    result = await client.embed([])
    assert result == []


async def test_embed_without_key_raises_runtime_error():
    """未配置真实 key 时调用 embed 应抛出 RuntimeError"""
    settings = Settings(embedding_dimensions=8)
    client = EmbeddingClient(settings)
...
async def test_embed_dimension_mismatch_still_returns(caplog):
    ...
    client = EmbeddingClient(settings)
    # 返回 3 维，配置期望 4 维
    client.client.embeddings.create = AsyncMock(return_value=_build_response([[0.1, 0.2, 0.3]]))
...
async def test_embed_propagates_api_errors():
    """底层 API 抛出异常时应向上传播"""
    settings = Settings(
        embedding_api_key="k",
        embedding_base_url="https://x.example.com/v1",
