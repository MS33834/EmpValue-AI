...
class TestJudgeEvidence:
    ...
    async def test_high_score_when_evidence_sourced(self):
        ...
        eval_result = {
            "employee_view": {
                "growth_areas": [
                    {"dimension": "技术交付", "evidence": ["主导完成用户画像模块重构，性能提升40%"]},
                    {"dimension": "团队协作", "evidence": ["辅导两名新人完成Code Review"]},
                ]
            }
        }
    ...
    async def test_low_score_when_evidence_missing(self):
        ...
        eval_result = {
            "employee_view": {
                "growth_areas": [
                    {"dimension": "技术交付", "evidence": ["虚构的业绩：完成月球基地建设"]},
                ]
            }
        }
        judge = MockJudge(None)
    ...
    async def test_partial_evidence_medium(self):
        ...
        eval_result = {
            "employee_view": {
                "growth_areas": [
                    {"dimension": "a", "evidence": ["主导完成用户画像模块重构，性能提升40%"]},
    ...
    async def test_high_score_when_constructive_and_sharp(self):
        ...
        eval_result = {
            ...
            "manager_view": {
                ...
                "risk_flags": [
                    {"level": "high", "category": "交付风险", "description": "需关注长期高负荷"}
                ],
            },
        }
        judge = MockJudge(None)
...
class TestJudgeHallucination:
    ...
    async def test_low_score_when_evidence_unsourced(self):
        ...
        eval_result = {
            "employee_view": {
                "growth_areas": [
                    {"dimension": "技术交付", "evidence": ["虚构业绩：完成月球基地建设"]},
                ]
            }
        }
        judge = MockJudge(None)
...
class TestJudgeAll:
    ...
    async def test_structure_and_overall_score_range(self):
        ...
        assert set(result.keys()) == {"evidence", "tone", "hallucination", "overall_score"}
...
class TestLLMJudgeParseScore:
    ...
    def test_parse_valid_json(self):
        result = LLMJudge._parse_score('{"score": 85, "reason": "证据充分"}', "evidence")
