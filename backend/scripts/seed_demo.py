...
SAMPLE_EVALUATION = {
        ...
        "growth_areas": [
            {
                "dimension": "交付质量与风险控制",
                "score": 65.0,
                "evidence": ["周一完成了订单中心核心接口重构，但自测不够充分，导致周二预发环境出现3处回归Bug，花了大半天定位修复"],
                "improvement_actions": [
            ...
            },
            {
                "dimension": "需求管理与沟通",
                "score": 70.0,
                "evidence": ["与产品经理在排期上发生争执，员工坚持要先补全测试用例再排新需求，双方最终达成一致由测试同学介入把关"],
                "improvement_actions": [
    ...
    "manager_view": {
        ...
        "hidden_issues": ["员工在与产品方争执时表现出的技术理想主义倾向，在高压时可能演变为协作障碍。"],
    },
    "audit": {
        "model_name": "gpt-4o-mini",
        "model_tier": "L0",
        "confidence_score": 0.7,
        "raw_data_refs": ["daily-001", "daily-002", "daily-003", "task-001", "meeting-001"],
...
async def main():
    ...
    async with AsyncSessionLocal() as session:
        ...
        raw_inputs = [
            RawInput(input_id="daily-001", employee_id="E1001", period="2026-W25", type="daily_report", content="周一完成了订单中心核心接口重构，但自测不够充分，导致周二预发环境出现3处回归Bug，花了大半天定位修复。"),
            RawInput(input_id="daily-002", employee_id="E1001", period="2026-W25", type="daily_report", content="周三主动组织跨团队对齐会，推动支付依赖方确认接口文档，最终把阻塞2天的JIRA-2051推进到联调阶段。"),
            RawInput(input_id="daily-003", employee_id="E1001", period="2026-W25", type="daily_report", content="周四周五集中做Code Review，辅导两名新人完成3个PR合并，并输出《接口变更 checklist》供团队复用。"),
            RawInput(input_id="task-001", employee_id="E1001", period="2026-W25", type="task_progress", content="客户侧反馈本次重构后接口延迟下降40%，但要求下周补齐幂等性和超时熔断文档。"),
            RawInput(input_id="meeting-001", employee_id="E1001", period="2026-W25", type="meeting_note", content="与产品经理在排期上发生争执，员工坚持要先补全测试用例再排新需求，双方最终达成一致由测试同学介入把关。"),
        ]
