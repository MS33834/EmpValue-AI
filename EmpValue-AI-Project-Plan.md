...
- [x] 选择 1-2 个团队进行内部试点（输出《试点 Runbook》含就绪清单 Go/No-Go、执行节奏、退出标准）
- [x] 收集主管与员工反馈（前端反馈/申诉记录面板 + `/employees/{id}/feedback` 端点追踪处理进度）
- [x] 建立反馈闭环，持续优化 Prompt（申诉→重评链路 + `evaluate.py --compare` 版本回归门禁）
- [x] 输出：《企业部署手册》《安全合规白皮书》《试点 Runbook》
