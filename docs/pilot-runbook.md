...
### 2.3 质量基线
- [ ] 回归评估集已通过：`python eval/evaluate.py --mock`（50/50 通过）。
- [ ] 真实模型回归已跑通：`python eval/evaluate.py --tier L0`（如有 API Key）。
...
2. **本地回归：** `python eval/evaluate.py --mock` 确认 50/50 通过。
3. **版本对比门禁：** 与上一发布版本对比，确认无回归：
   ```bash
   python eval/evaluate.py --mock --compare v0.1 --output reports/compare_v0.2.json
   ```
