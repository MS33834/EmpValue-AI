      ...
      <el-tabs v-model="activeTab" class="login-tabs">
        <el-tab-pane label="账号登录" name="jwt">
          ...
          >
            ...
            <div class="demo-tip">
              演示账号：employee@empvalue.ai / empvalue123
...
const jwtForm = reactive({
  email: 'employee@empvalue.ai',
  password: 'empvalue123',
})
...
async function handleJwtLogin() {
  ...
  try {
    ...
    router.push(redirectMap[res.role] || '/employee')
  } catch (err) {
...
async function seedDemoUsers() {
  ...
  try {
    ...
    ElMessage.success(`演示账号已就绪：${res.created.length} 个新建，默认密码 ${res.default_password}`)
  } catch (err) {
